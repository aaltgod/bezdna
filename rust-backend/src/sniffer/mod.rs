...
use crate::repository::db::postgres::flags as flags_repo;
...
pub struct Sniffer {
    ...
    flags_repo: flags_repo::Repository,
...
impl Sniffer {
    ...
    async fn manage_tcp_streams(&self) {
        loop {
            ...
            for (port_pair, tcp_packet_info) in tcp_packet_info_by_port_pair.iter() {
                if !tcp_packet_info.is_empty() {
                    ...
                    if time_now
                        ...
                        || tcp_packet_info.last().unwrap().completed
                        || time_now
            ...
            let mut streams_to_insert: Vec<domain::Stream> =
                Vec::with_capacity(completed_stream_port_pairs.len());

            for pair in completed_stream_port_pairs {
                let tcp_packet_info = tcp_packet_info_by_port_pair.get(&pair).unwrap();

                let mut payload = String::new();
                let mut started_at: chrono::DateTime<chrono::Utc> = Default::default();
                let mut ended_at: chrono::DateTime<chrono::Utc> = Default::default();

                for (idx, info) in tcp_packet_info.iter().enumerate() {
                    if idx == 0 {
                        started_at = info.at
                    }

                    if idx.eq(&(tcp_packet_info.len() - 1)) {
                        ended_at = info.at
                    }

                    payload.push_str(info.payload.as_str())
                }

                if !payload.is_empty() {
                    streams_to_insert.push(domain::Stream {
                        id: 0,
                ...
                // TODO: удалить из памяти только после
                // успешной вставки в БД
                tcp_packet_info_by_port_pair.remove(&pair);
            }

            if !streams_to_insert.is_empty() {
                self.streams_repo
                    .insert_streams(streams_to_insert)
                    .await
                    .unwrap();
            }
        }
